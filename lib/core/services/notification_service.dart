...
class NotificationService {
  ...
  void applyGlobalSettings({
    ...
    _volume = volume;
    debugPrint('⚙️ Notification settings applied: '
        'enable=$_enableNotifications, sound=$_soundEnabled, vibration=$_vibrationEnabled, volume=$_volume');
  }

  Future<void> scheduleNotification(Medicine medicine, {bool dailyRepeat = false, TimeOfDay? reminderTime}) async {
  ...
  if (reminderTime != null) {
    ...
    if (scheduledDate.isBefore(now)) {
      // Optional: show immediately if already passed
  ...
  debugPrint('⏰ Scheduled notification for ${medicine.name} at $scheduledDate');
}
